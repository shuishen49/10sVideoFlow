...
const { createDubbingVoiceHandlers } = require('../handlers/dubbing-voice');
...
function loadSkillEnvIntoProcess() {
  const roots = [
    path.resolve(PROJECT_ROOT, '..'), // workspace/skill (legacy)
    path.resolve(PROJECT_ROOT, '..', 'ocSkill'), // workspace/ocSkill (legacy)
    path.resolve(PROJECT_ROOT, '..', 'gitskill'), // workspace/gitskill
    path.resolve(PROJECT_ROOT, '..', 'gitskill', 'video-generation-pic2api'), // explicit preferred for PIC2API_KEY
    path.resolve(PROJECT_ROOT, '..', 'gitskill', 'linggan-video-tools'),
    path.resolve(PROJECT_ROOT, '..', 'video-generation-pic2api'), // sibling under gitskill
    path.resolve(PROJECT_ROOT, '..', 'linggan-video-tools'), // sibling under gitskill
  ];
  ...
  // Prefer dedicated pic2api skill key when multiple .env files define PIC2API_KEY.
  try {
    const preferredPic2EnvPath = path.resolve(PROJECT_ROOT, '..', 'video-generation-pic2api', '.env');
    if (fs.existsSync(preferredPic2EnvPath)) {
      const preferredKv = parseDotEnv(fs.readFileSync(preferredPic2EnvPath, 'utf8'));
      const preferredPic2Key = String(preferredKv.PIC2API_KEY || '').trim();
      if (preferredPic2Key) process.env.PIC2API_KEY = preferredPic2Key;
    }
  } catch (err) {
    console.warn('[bridge] apply preferred PIC2API_KEY failed:', err.message);
  }
}
...
const CONFIG_PATH = process.env.STORYBOARD_BRIDGE_CONFIG || path.join(PROJECT_ROOT, 'bridge-config.json');
...
const PREVIEW_ASSETS_ROOT = process.env.STORYBOARD_ASSETS_ROOT || path.join(PROJECT_ROOT, 'assets');
...
const LINGGAN_SCRIPT_PATH = pickFirstExisting([
  path.resolve(PROJECT_ROOT, '..', 'linggan-video-tools', 'scripts', 'call_generate_character_image.py'), // gitskill sibling
  path.resolve(PROJECT_ROOT, '..', 'gitskill', 'linggan-video-tools', 'scripts', 'call_generate_character_image.py'), // explicit gitskill
  path.resolve(PROJECT_ROOT, '..', 'ocSkill', 'linggan-video-tools', 'scripts', 'call_generate_character_image.py'), // legacy
]);
const LINGGAN_SCENE_SCRIPT_PATH = pickFirstExisting([
  path.resolve(PROJECT_ROOT, '..', 'linggan-video-tools', 'scripts', 'call_generate_shot_image_with_refs.py'), // gitskill sibling
  path.resolve(PROJECT_ROOT, '..', 'gitskill', 'linggan-video-tools', 'scripts', 'call_generate_shot_image_with_refs.py'), // explicit gitskill
  path.resolve(PROJECT_ROOT, '..', 'ocSkill', 'linggan-video-tools', 'scripts', 'call_generate_shot_image_with_refs.py'), // legacy
]);
const PIC2API_SCRIPT_PATH = pickFirstExisting([
  path.resolve(PROJECT_ROOT, '..', 'video-generation-pic2api', 'scripts', 'generate_video.py'), // gitskill sibling
  path.resolve(PROJECT_ROOT, '..', 'gitskill', 'video-generation-pic2api', 'scripts', 'generate_video.py'), // explicit gitskill
]);
...
function loadConfig() {
  ...
  const dubbing = (file && typeof file.dubbing === 'object' && file.dubbing) ? file.dubbing : {};
  return {
    ...
    dubbing: {
      enabled: process.env.STORYBOARD_DUBBING_ENABLED
        ? ['1', 'true', 'yes', 'on'].includes(String(process.env.STORYBOARD_DUBBING_ENABLED).trim().toLowerCase())
        : dubbing.enabled !== false,
      provider: String(process.env.STORYBOARD_DUBBING_PROVIDER || dubbing.provider || 'dashscope').trim().toLowerCase() || 'dashscope',
      baseUrl: String(process.env.STORYBOARD_DUBBING_BASE_URL || process.env.DASHSCOPE_BASE_URL || dubbing.baseUrl || 'https://dashscope.aliyuncs.com').trim().replace(/\/$/, ''),
      designModel: String(process.env.STORYBOARD_DUBBING_DESIGN_MODEL || dubbing.designModel || 'qwen-voice-design').trim() || 'qwen-voice-design',
      ttsModel: String(process.env.STORYBOARD_DUBBING_TTS_MODEL || dubbing.ttsModel || 'qwen3-tts-vd-realtime-2026-01-15').trim() || 'qwen3-tts-vd-realtime-2026-01-15',
      language: String(process.env.STORYBOARD_DUBBING_LANGUAGE || dubbing.language || 'zh').trim() || 'zh',
    },
    guard: {
...
async function handleVideoGenerate(req, res, bodyText) {
  ...
  const tailImageDataUrl = String(body?.imageTailDataUrl || body?.tailImageDataUrl || '').trim();
  const headImagePath = headImageRaw ? resolveSceneRefPath(headImageRaw) : '';
  let tailImagePath = tailImageRaw ? resolveSceneRefPath(tailImageRaw) : '';
  ...
  if (tailImageDataUrl) {
    if (!/^data:image\/[\w.+-]+;base64,/i.test(tailImageDataUrl)) {
      return sendJson(res, 400, { error: { type: 'tail_image_invalid_data_url', message: '尾帧图 data URL 非法，需 data:image/*;base64,...' } });
    }
    const comma = tailImageDataUrl.indexOf(',');
    const meta = tailImageDataUrl.slice(0, comma);
    const b64 = tailImageDataUrl.slice(comma + 1);
    const extFromMeta = (meta.match(/^data:image\/([\w.+-]+);base64$/i)?.[1] || 'png').toLowerCase();
    const extMap = { jpg: '.jpg', jpeg: '.jpg', png: '.png', webp: '.webp', gif: '.gif', bmp: '.bmp' };
    const ext = extMap[extFromMeta] || '.png';
    let buf;
    try {
      buf = Buffer.from(b64, 'base64');
    } catch {
      return sendJson(res, 400, { error: { type: 'tail_image_invalid_base64', message: '尾帧图 base64 解析失败' } });
    }
    if (!buf || !buf.length) {
      return sendJson(res, 400, { error: { type: 'tail_image_empty', message: '尾帧图为空' } });
    }
    const maxBytes = 5 * 1024 * 1024;
    if (buf.length > maxBytes) {
      return sendJson(res, 400, { error: { type: 'tail_image_too_large', message: '尾帧图不能超过 5MB' } });
    }
    const tmpDir = path.join(PROJECT_ROOT, 'tmp', 'video-tail-refs');
    fs.mkdirSync(tmpDir, { recursive: true });
    const tmpName = `${sanitizeFileStem(`${project || 'project'}-${segmentId || 'segment'}-tail`)}-${Date.now()}${ext}`;
    const tmpPath = path.join(tmpDir, tmpName);
    fs.writeFileSync(tmpPath, buf);
    tailImagePath = tmpPath;
  }

  if (tailImagePath && /^https?:\/\//i.test(tailImagePath)) {
    return sendJson(res, 400, { error: { type: 'tail_remote_image_unsupported', message: '尾帧图仅支持本地路径或 data URL' } });
  }
  ...
  if (tailImageDataUrl && tailImagePath && path.isAbsolute(tailImagePath) && fs.existsSync(tailImagePath)) {
    try { fs.unlinkSync(tailImagePath); } catch {}
  }
...
} = createDubbingVoiceHandlers({ loadConfig, sendJson, callJson });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/dubbing/voice/design') {
      const bodyText = await collectBody(req);
      return handleDubbingVoiceDesign(req, res, bodyText);
    }

    if (req.method === 'POST' && url.pathname === '/api/dubbing/voice/preview') {
      const bodyText = await collectBody(req);
      return handleDubbingVoicePreview(req, res, bodyText);
    }
